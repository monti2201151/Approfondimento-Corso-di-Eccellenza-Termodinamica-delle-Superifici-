...
# =======================================================================
# MAIN // TEMPLATE GENERICO PER UN CRISTALLO QUALSIASI (sistema cubico)
# =======================================================================
#
# Per usare questo script con un altro materiale/dataset, e' sufficiente
# modificare le variabili in questa sezione: il resto del codice (fasi
# 1-8) resta invariato.
 
if __name__ == "__main__":
 
    # -------------------------------------------------------------
    # 1. Identificazione del cristallo (solo per stampa/titolo grafico)
    # -------------------------------------------------------------
    CRYSTAL_NAME = "Nome del cristallo (es. materials-id)"
 
    # -------------------------------------------------------------
    # 2. Parametro di reticolo cubico, in Angstrom
    #    (None -> usa il caso cubico "unitario" con la scorciatoia
    #    n // (h,k,l); passare un valore reale se serve la scala fisica
    #    assoluta, es. per confronti quantitativi tra materiali diversi)
    # -------------------------------------------------------------
    LATTICE_PARAMETER = None   # es. 4.08 per l'oro fcc (Angstrom)
    lattice_matrix = cubic_lattice(LATTICE_PARAMETER) if LATTICE_PARAMETER else None
 
    # -------------------------------------------------------------
    # 3. Famiglie cristallografiche con la relativa energia superficiale
    #    gamma_hkl (unita' arbitrarie o J/m^2, a seconda della sorgente
    #    dei dati). Sostituire con i valori del materiale di interesse,
    #    tipicamente ottenuti da calcoli DFT o dalla letteratura.
    # -------------------------------------------------------------
    families = [
        MillerFamily(hkl=(1, 0, 0), gamma=1.00),
        MillerFamily(hkl=(1, 1, 0), gamma=1.10),
        MillerFamily(hkl=(1, 1, 1), gamma=0.90),
        # aggiungere altre famiglie qui, es.:
        # MillerFamily(hkl=(2, 1, 0), gamma=1.05),
        # MillerFamily(hkl=(2, 1, 1), gamma=0.95),
    ]
 
    # -------------------------------------------------------------
    # 4. Costante di scala k (arbitraria: fissa solo le dimensioni
    #    assolute del poliedro, non la sua forma)
    # -------------------------------------------------------------
    K_SCALE = 1.0
 
    # -------------------------------------------------------------
    # Esecuzione: fasi 4-7 (costruzione del poliedro)
    # -------------------------------------------------------------
    result = build_wulff_polyhedron(families, k=K_SCALE, lattice_matrix=lattice_matrix)
 
    # -------------------------------------------------------------
    # Riepilogo testuale
    # -------------------------------------------------------------
    print(f"Cristallo: {CRYSTAL_NAME}")
    print("Famiglie cristallografiche considerate:")
    for fam in families:
    ...
    fig, ax, check = plot_wulff_shape(
        ...
        title=f"Wulff shape of {CRYSTAL_NAME}",
    )
